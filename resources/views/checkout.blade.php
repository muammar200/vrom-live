      ...
      <div class="container">
          ...
          <div class="flex items-center gap-5 lg:justify-between">
              ...
              {{-- <form action="{{ route('front.checkout.store', $item->slug) }}" class="bg-white p-[30px] pb-10 rounded-3xl max-w-[490px] w-full" x-data="app"
                  x-cloak id="checkoutForm" method="POST"> --}}
              <form action="{{ route('front.checkout.store', $item->slug) }}" class="bg-white p-[30px] pb-10 rounded-3xl max-w-[490px] w-full" x-data="app"
                   id="checkoutForm" method="POST" x-cloak>
                  @csrf
                  @method('post')
                  <div class="grid grid-cols-2 items-center gap-y-6 gap-x-4 lg:gap-x-[30px]">
                      ...
                      <div class="flex flex-col col-span-1 gap-3">
                          ...
                          <input value="{{ old('name') ?? '' }}"  type="number" name="zip" id="postCode"
                              class="text-base font-medium focus:border-primary focus:outline-none placeholder:text-secondary placeholder:font-normal px-[26px] py-4 border border-grey rounded-[50px] focus:before:appearance-none focus:before:!content-none"
                      ...
                      {{-- <div class="col-span-2 mt-[26px]">
                          <button type="submit" class="block w-full px-6 py-3 font-bold text-center text-white rounded-full bg-primary">
                              Continue
                          </button>
                      </div> --}}
              ...
              <img src="{{ $item->thumbnail }}" class="max-w-[50%] hidden lg:block -mr-[100px]"
                  alt="">
          </div>
      </div>
  </section>
